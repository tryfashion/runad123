...
**当前阶段：M0–M6 本地实现完成，注册改为管理员审核＋密码登录，SMTP 暂不接入。最近注册增量验证：134 项单元、4 条浏览器用例通过；本轮 MySQL 迁移实测受本机服务权限限制待完成。真实 DeepSeek、Shopify 导入及生产运维仍待验证，详见实施计划。**
...
Chrome 打开 `chrome://extensions`，开启开发者模式，加载 `D:\aiproject\runad123\apps\extension\dist`。首次打开直接显示产品首页；打开 Shopify **商品详情页**，点击采集即可本地预览，无需先登录。选择目标国家后，点击带数据使用说明的“创建草稿”按钮才建立匿名身份并上传。账号表单位于右上角“账号 / 登录”的独立视图。可以查看界面、编辑草稿、查看后台统计和管理教程。AI 风险检查、改写及依赖有效检查的正式 CSV 导出保持不可用；普通账号使用申请审核与密码登录。插件不设置 manifest key，ID 由 Chrome 自动分配；本机预览无需填写 CHROME_EXTENSION_IDS，正式环境仍使用准确的 ID 白名单。
...
登录账号后，网站概览通过扩展后台携带会话向自家 API 查询当前域名的注册日期、年龄、到期日及注册商，插件不直接访问第三方 RDAP 服务，不新增第三方域名权限。服务端成功结果进程内缓存 7 天，失败缓存 5 分钟，重启失效；未公开字段显示未识别。普通网站每日缓存与手动读取保持不变；域名信息不持久缓存到插件，退出登录后隐藏，手动读取复用服务端有效域名缓存。启用需同时更新网站/API 和重新构建、加载插件，无数据库迁移。域名年龄不是店铺开业时间。

## 注册申请与管理员审核

暂不接入 SMTP。插件域名卡片或账号页点击“注册 / 登录”，在自家网站独立窗口填写邮箱、密码、确认密码及使用目的，提交后等待审核。网站也可直接访问 /account 申请或登录。邮箱当前仅作为登录名，未验证归属，不用于自动找回密码或关联旧账号。

管理员进入“账号管理 → 注册审核”，查看待审核申请并通过或拒绝，可附内部备注。审核前不创建可登录用户、不签发账号会话；通过后创建普通用户。用户再次打开窗口，用设置的密码登录，插件自动返回原标签并读取域名信息。无邮件通知；拒绝后需联系管理员，本版无自助重提、找回密码或普通用户密码修改。内部管理员继续使用 /admin/login。

密码两次输入，服务端使用 scrypt salt/hash；注册按 IP、邮箱和全局限速，登录按 IP、账号限速，审核只允许管理员网站会话并检查 CSRF、写审计。已注册邮箱的重复提交返回统一结果，不修改原密码。旧邮箱验证码 API 已关闭，不可绕过审核；强制登录开关不再依赖 SMTP。已有邮箱账号不会因相同邮箱的新申请被自动接管；既有管理员登录不受影响。

插件窗口仅允许自家 origin、精确路径、顶层 frame、预绑定 tab 和 30 分钟 nonce。网页不接收插件令牌，刷新仅保留邮箱与用途，不保存密码；本版不自动关联匿名历史。界面支持简中、繁中、英语。Google 登录未接入。

**需要更新服务端并执行 pnpm db:migrate，应用新增 0009_member_review.sql，然后构建网站、重启服务并重新构建/加载插件。** 服务器仍只运行网站/API 和 worker，插件包供用户浏览器安装。开发迁移与生产发布的验证状态见 IMPLEMENTATION_PLAN。

独立 Chromium 回归：构建网站后，以 RUNAD_API_ORIGIN=http://127.0.0.1:3107 构建扩展至 artifacts/registration-extension，运行 node scripts/run-playwright.mjs test --config tests/registration.config.ts。网站与扩展交互使用真实浏览器；API/审核存储按用例模拟。截图输出 artifacts/registration-application.png、registration-pending.png、registration-login.png。