...
// Chrome serializes only this function; all runtime dependencies must live inside it.
export async function collectPage(
  mode: 'product' | 'collection' = 'product',
  selectedHandles?: string[],
): Promise<ReadResult & CollectionResult> {
  ...
  async function collectCurrentProduct() {
    ...
    try {
      ...
      if (
        document.querySelector(
          '[name^="properties["][required],input[name="selling_plan"][required]',
        )
      )
  ...
  async function collectCollection(): Promise<CollectionResult> {
    const state = globalThis as typeof globalThis & { runadCollector?: AbortController };
    state.runadCollector?.abort();
    const controller = new AbortController();
    state.runadCollector = controller;
    ...
    try {
      ...
      const root =
        url.origin +
        (match?.[1] ?? /^\/[a-z]{2}(?:-[A-Za-z]{2})?(?=\/|$)/.exec(url.pathname)?.[0] ?? '') +
        '/';
      const before = await readCurrency(root, controller.signal);
      ...
      const listing = (
        selectedHandles
          ? { products: selectedHandles.map((handle) => ({ handle })) }
          : JSON.parse(await readText(collectionEndpoint, controller.signal))
      ) as {
  ...
  return mode === 'collection' ? collectCollection() : collectCurrentProduct();
}
