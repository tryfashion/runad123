...
- 邮箱验证码注册/登录共用流程。网站使用 HttpOnly/Secure/SameSite=Lax cookie；插件使用 Bearer 令牌。原始令牌只交付一次，数据库存 SHA-256 摘要。
- 首版会话续期延长有效期但不更换令牌，需原会话仍有效、账号未禁用；响应丢失可安全重试。退出/禁用明确撤销，过期重新认证。暂不实现“立即撤销旧 token 的自动轮换”，避免弱网下丢失匿名身份。
- 验证码存带服务端密钥的 HMAC，10 分钟过期、最多 5 次验证，重发间隔 60 秒；发送和验证都按邮箱、安装及短期 IP 指纹限速。验证码不写日志。
- 邮件通过有 10 秒超时的服务端适配器发送；挑战先置 pending，供应商接受后置 sent，只有 sent 可验证。失败允许受限重发；发送成功而响应丢失时可能多封邮件，以最新可用 challenge 为准。首版不为邮件引入另一套持久队列。
- 新用户默认为普通角色；管理员只能通过运维初始化，不允许公开注册时指定角色。
- 登录时只在用户勾选同意后关联该安装的历史采集；不同匿名身份不能凭客户端提交 ID 合并。
...
- 后台开启强制登录前校验邮件服务已配置；管理员必须完成一次真实收码测试。首版后台只允许改白名单配置，不能编辑任意环境变量或密钥。
