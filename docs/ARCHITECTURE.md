...
当前采集优先版产品页：全部商品/当前商品/按系列导出 → 搜索与系列筛选、创建时间/价格排序 → 商品卡片（列表/双列网格）→ 勾选导出、单品导出与加载更多。每页读取5件公开商品；搜索排序只针对已读取项并标明范围。无热销排名。点击导出直接下载本地 CSV，不要求登录；全店/系列导出独立分页枚举，每批10件复用带币种核验的采集器，单文件最多1000件，超限明确拒绝且不冒充完整导出。旧预览、草稿恢复、编辑/风险/改写/许可导出界面已删除；产品入口仅保留新版目录、加载状态、非 Shopify 提示及后台配置的引流网站、授权引导。服务端草稿及 AI 接口保留。
...
- 注册提交邮箱和密码两次，用途默认隐藏且可省略（REGISTRATION_PURPOSE_ENABLED=true 恢复表单），经管理员审核才创建普通账号，用户随后凭密码登录；暂不接入 SMTP。邮箱未验证，不自动绑定既有用户。网站使用 HttpOnly/Secure/SameSite=Lax cookie；插件使用 Bearer 令牌。原始令牌只交付一次，数据库存 SHA-256 摘要。
...
- 后台开启强制登录无需邮件服务；须先完成注册审核迁移及审核账号的真实登录验收。后台只允许改白名单配置；AI密钥可在专用管理接口中加密保存，不能编辑任意环境变量。
...
M3 实现：Next 负责创建/读取/取消/显式重试；独立 Node worker 轮询 MySQL 任务并续租、记 attempt、调用 DeepSeek 和结算；配置及价格在 settings.ai_risk，API key 可由管理员在系统管理 → AI配置管理中保存（AES-256-GCM加密至settings.ai_providers），worker运行时解密；旧DEEPSEEK_API_KEY环境配置保留兼容。无配置保持关闭。插件关闭只停止轮询；新开从草稿取 request 恢复。具体当前验收见 M3_VALIDATION.md，真实模型和质量评估未通过前不宣称生产可用。
...
- 单请求超时初始 90 秒，默认最多3次总尝试，后台可设置1–5次，仅重试超时、限流、服务错误或一次结构化输出修复；使用退避和抖动。确定的参数/鉴权错误直接失败。
- 字符数限制不是 token 上限：适配器须同时配置输入 token 预算、输出上限和上下文余量。风险结果与改写 HTML 分别设输出上限；无法容纳完整文本时提前返回不支持，不能靠截断完成检查。每次真正发出调用前持久记录 attempt，崩溃恢复也不得重置任务快照中的尝试次数上限。
...
最多保存20套Chat Completions兼容API配置，每类用途只选择一套生效配置；DeepSeek仍受支持。支持自定义公网HTTPS地址（443端口、IPv4）、接口路径、模型、两类补充规则、超时5–120秒、尝试1–5次、退避基数1–60秒、Temperature、Token上限及USD价格预算。单worker仍串行处理，不展示未实现的并发控制。原生Anthropic/Gemini等不同协议不在本轮范围；图片风险检测仍待后续实施。

密钥使用AUTH_SECRET派生专用加密密钥，AES-256-GCM随机nonce及profile ID附加验证；列表仅返回hasKey，日志/审计/任务快照无密钥。更换接口地址或路径须重新输入密钥；AUTH_SECRET轮换后须重新配置密钥。保存用expectedVersion防止覆盖，并记录不含规则正文/密钥的审计。自定义请求先解析并固定公网IPv4，拒绝内网/保留地址、跳转、URL凭据和查询参数，响应限1MiB。检查配置只调用/models验证认证和模型可见性，不发起生成、不发送商品；不等价于真实业务生成验收。

任务快照记录providerId/providerRevision及非敏感调用参数；配置变更使旧缓存失效，旧排队任务遇到配置版本不符明确失败，不能静默切换提供方。费用预留乘任务最大尝试次数，风险与改写仍共用风险配置每日预算。worker无需重启即可读取保存配置；初次部署新worker代码仍需重启。自定义规则不得覆盖内置结构化输出、事实保真和非法律判定约束。
